...
import UpdateDialog from './dialog/registerToday/updateDialog.vue';
...
export default {
  ...
  data() {
    return {
      ...
      nowTime: parseTime(new Date(), 'yyyy-MM-dd'),
      showDialog: false,
      visible: false,
      tableData: [],
      updateData: {},
  ...
  methods: {
    backTime() {
      this.query.date = parseTime(new Date(), 'yyyy-MM-dd');
      this.getPage();
    },
    back() {
      this.$emit('back');
