<template>
  <div class="drawer">
    ...
    <div class="desc">
      <section>
        ...
        <ul class="detail">
          ...
          <li><span>预警人电话:</span> {{ petitionInfo.personTelephone }}</li>

          <li>
            <span>诉求类别:</span>
            {{ petitionInfo.infoClassifyText }}
          </li>
          <li>
            <span>信息渠道:</span>
            {{ petitionInfo.infoChannelText }}
          </li>
          <li><span>登记人:</span> {{ petitionInfo.reportUser }}</li>
          <li><span>登记人电话:</span> {{ petitionInfo.petitionPhone }}</li>
          <li><span>登记单位:</span> {{ petitionInfo.departName }}</li>
          <li>
            <span>登记级别:</span>
            {{ petitionInfo.infoSourceText }}
          </li>
          <li>
            <span>紧急程度:</span>
            {{ petitionInfo.infoLevelText }}
          </li>
          ...
          <li class="line">
            ...
            <a
              ...
              target="_blank"
              class="attach"
              v-for="attach in petitionInfo.filePath"
              :key="attach.id"
              >{{ attach.name }}</a
            >
          </li>
          <li class="repeat line" v-for="(repeatInfo, index) in petitionInfoRepeatList" :key="repeatInfo.id">
            <header>
              ...
              <el-button v-if="petitionInfo.state < 2 && notFeedback" @click="repeatEdit(repeatInfo)">编 辑</el-button>
            </header>
            <ul class="detail">
              <li><span>登记人:</span> {{ repeatInfo.reportUser }}</li>
              <li><span>联系电话:</span> {{ repeatInfo.reportPhone }}</li>
              <li><span>登记日期:</span> {{ repeatInfo.petitionDate }}</li>
              <li><span>登记级别:</span> {{ repeatInfo.infoSourceName }}</li>
              ...
              <li class="line">
                ...
                <a
                  ...
                  target="_blank"
                  class="attach"
                  v-for="attach in repeatInfo.filePath"
                  :key="attach.id"
                  >{{ attach.name }}</a
                >
              </li>
    ...
    <footer>
      <el-button type="text">
        <el-button type="text" v-if="petitionInfo.permType === 1">
          <el-button @click="backDialog = true" type="default">反 馈</el-button>

          <el-button type="text">
            <el-popconfirm title="确定要解除预警吗？" @confirm="deleteRow">
              <el-button slot="reference" type="primary">解除预警</el-button>
            </el-popconfirm>
          </el-button>
        </el-button>
        <el-button type="text" v-if="petitionInfo.permType === 2">
          <el-button type="text">
            <el-popconfirm title="确定要开启预警吗？" @confirm="deleteRow">
        ...
        <el-button type="text" v-if="petitionInfo.permType === 3">
          <el-button @click="backDialog = true" type="default">反 馈</el-button>
          <el-button type="text">
...
<script>
// import LetterContentForm from '@/views/rightMaintaince/dialog/abnormalInfo/letterContentForm.vue';
  ...
  methods: {
    ...
    handleDelete() {
      this.$confirm('确定删除该异动吗?', '提示', {
        ...
        axios.get(`/petitionInfo/delete?id=` + this.detailId).then((res) => {
    ...
    sendTo(flowType) {
      ...
      this.activeInfo.flowType = flowType;
      switch (flowType) {
        case '1':
          this.showUp = true;
          break;
    ...
    handleClose(tag, index) {
      let id = this.petitionInfo.helpPersonId && this.petitionInfo.helpPersonId.split(',');
      let name = this.petitionInfo.helpPersonName && this.petitionInfo.helpPersonName.split(',');
      name.splice(index, 1);
      id.splice(index, 1);
      axios({
        url: '/petitionInfo/update',
        ...
        data: {
          id: this.petitionInfo.id,
          helpPersonId: id.join(','),
          helpPersonName: name.join(','),
          flowType: 0,
        },
      }).then((res) => {
        this.$message.success('操作成功');
    ...
    deleteRow() {
      axios({
        url: '/petitionWarning/openOrClose',
        method: 'get',
        params: {
          id: this.$route.query.id,
        },
    ...
    openRow() {
      axios({
        url: '/petitionWarning/trackOnOrOff',
        method: 'get',
        params: {
          id: this.$route.query.id,
        },
