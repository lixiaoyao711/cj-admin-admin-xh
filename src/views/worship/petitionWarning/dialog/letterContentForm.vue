  ...
  <div class="">
    ...
    >
      <el-form :model="form" :rules="rules" ref="form" class="form" label-width="100px" :inline="false">
        ...
        <el-col :span="12">
          <el-form-item label="信访目的:" prop="infoType">
            <el-select v-model="form.infoType" @change="infoTypeChange">
              <el-option
                v-for="item in dictMap.info_type"
                :key="item.dictCode"
        ...
        <el-col :span="12">
          <el-form-item v-if="form.infoType" label="诉求类别:" prop="infoClassify">
        ...
        <el-col :span="12">
          <el-form-item label="信息等级:" prop="infoLevel">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button v-if="!info.id && !info.disposeFlag" @click="sendTo('1')">上 报</el-button>
        <el-button v-if="!info.id && !info.disposeFlag" @click="sendTo('2')">下 派</el-button>
        <el-button v-if="!info.id && !info.disposeFlag" @click="sendTo('9')">转 交</el-button>
        <!-- disposeFlag 自行处置(0:否1:是) -->
        <el-button type="primary" @click="handleSave">{{ !!info.disposeFlag ? '办 结' : '保 存' }}</el-button>
      </span>
    </el-dialog>
    <AddLetterPeople
...
export default {
  ...
  methods: {
    getDetail() {
      axios({
        methods: 'get',
        url: '/petitionInfo/getById',
        params: { id: this.info.id },
      }).then((res) => {
        console.log('detailId', res);
        this.form = Object.assign({}, res.petitionInfo);
        // this.form.name = this.petitionInfo.name
        this.form.retireIds = res.petitionInfoRetireList.map((e) => e.personId).join(',');
        this.form.retireOutIds = res.petitionInfoRetireOutList.map((e) => e.personId).join(',');
        // this.form.departCode = setFormDepartCode(this.form.departCode);
        console.log('this.form.departCode', this.form.departCode);
        this.form.petitionDate = this.form.petitionDate || new Date();
        this.fileList = res.petitionInfo.filePath || [];
        // 手动触发下拉框时间
        // this.infoSourceChange(this.info.infoSource);
        // this.infoTypeChange(this.info.infoType);
    ...
    initForm() {
      ...
      if (this.people.length) {
        ...
        this.form.name = this.people.map((e) => e.name).join(',');
        ...
        this.form.name = this.people.name;
    ...
    addPeopleSuccess(selection) {
      ...
      this.form.name = selection.map((e) => e.name).join(',');
      this.form.infoDepartCode = selection[0].departCode;
      this.form.infoDepartName = selection[0].departName;
      let deleteRetireIds = this.form.retireIds.split(',');
      let deleteRetireOutIds = this.form.retireOutIds.split(',');
      let retireIds = selection.filter((e) => e.isOut === 0).map((e) => e.id);
      let retireOutIds = selection.filter((e) => e.isOut === 1).map((e) => e.id);
      // selection.forEach((e, i) => {
      //   if (e.isOut === 0) {
      //     // 户内
      //     let include = deleteRetireIds.indexOf(e.id);
      //     if (include > -1) {
      //       deleteRetireIds.splice(include, 1);
      //       retireIds.splice(i, 1);
      //     }
      //   } else {
      //     let include = deleteRetireOutIds.indexOf(e.id) > -1;
      //     if (include) {
      //       deleteRetireOutIds.splice(include, 1);
      //       retireOutIds.splice(i, 1);
      //     }
      //   }
      // });
      this.form = Object.assign(this.form, {
        // deleteRetireIds: deleteRetireIds.join(','),
        // deleteRetireOutIds: deleteRetireOutIds.join(','),
        retireIds: retireIds.join(','),
    ...
    updateDeadline() {
      ...
      this.form.deadline = new Date(stamp + deadlineMap[this.form.infoLevel]).dateFormat('yyyy-MM-dd');
    },
    validate(type) {
      ...
      return new Promise((resolve, reject) => {
        this.$refs['form'].validate((valid) => {
          ...
          let formData = Object.assign({}, this.form, depart);
          resolve(formData);
        });
      });
    },
