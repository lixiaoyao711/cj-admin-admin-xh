<template>
  <div class="workInfoEditDialog">
    <el-dialog title="附件信息" :visible.sync="open" width="1080px" append-to-body @close="cancel">
      <el-form ref="form" :model="form.expandContent" :rules="rules" label-width="140px">
        <el-form-item label="身份证" prop="">
          <imageUpload v-model="form.expandContent.idCardUrl" :fileSize="20"></imageUpload>
        </el-form-item>
        <el-form-item label="户口登记薄" prop="">
          <imageUpload v-model="form.expandContent.householdRegBookUrl" :fileSize="20"></imageUpload>
        </el-form-item>
        <el-form-item label="奖惩情况" prop="">
          <imageUpload v-model="form.expandContent.rewardsAndPunishmentsUrl" :fileSize="20"></imageUpload>
        </el-form-item>
        <el-form-item label="信息表签字" prop="">
          <imageUpload v-model="form.expandContent.formationVisaFormUrl" :fileSize="20"></imageUpload>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitForm">确 定</el-button>
        <el-button @click="cancel">取 消</el-button>
      </div>
    </el-dialog>
  </div>
</template>

<script>
import { getPersonExpandById, addPersonExpand, updatePersonExpand } from '@/api/worship/retiredSoldier';
export default {
  props: {
    propsId: '',
    title: {
      type: String,
      default: '',
    },
  },
  dicts: ['sys_worship_educationlevel', 'sys_worship_academicdegree'],
  components: {},
  data() {
    return {
      open: true,
      form: {
        expandChildType: 'A7',
        expandParentType: 'A',
        expandContent: {
          idCardUrl: '',
          householdRegBookUrl: '',
          rewardsAndPunishmentsUrl: '',
          formationVisaFormUrl: '',
        },
      },

      rules: {
        educationBackground: [{ required: true, message: '学历情况不能为空', trigger: 'blur' }],
        // name: [{ required: true, message: '姓名不能为空', trigger: 'blur' }],
      },
    };
  },
  computed: {},
  watch: {},
  created() {},
  mounted() {
    this.getData();
  },
  methods: {
    getData() {
      if (this.propsId) {
        getPersonExpandById(this.propsId).then((res) => {
          this.form = res.data;
          this.form.expandContent = JSON.parse(this.form.expandContent);
        });
      }
    },
    submitForm() {
      this.$refs['form'].validate((val) => {
        if (val) {
          this.form.expandContent = JSON.stringify(this.form.expandContent);
          this.form.personId = this.$route.params.id;

          if (this.propsId) {
            updatePersonExpand(this.form).then((res) => {
              this.$message.success('修改成功');
              this.$emit('success');
              this.$emit('close');
            });
          } else {
            addPersonExpand(this.form).then((res) => {
              this.$message.success('添加成功');
              this.$emit('success');
              this.$emit('close');
            });
          }
        }
      });
    },
    cancel() {
      this.$emit('close');
    },
  },
};
</script>

<style scoped lang="scss">
.workInfoEditDialog {
}
.icon {
  cursor: pointer;
  color: red;
  margin: 5px 0 0 16px;
  font-size: 20px;
}
.peopleList {
  display: flex;
  // justify-content: space-between;
  .item {
    // flex: 20%;
    width: 46%;
  }
}
</style>
