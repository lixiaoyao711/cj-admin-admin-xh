<template>
  <div class="indoor">
    ...
    <div class="table">
      ...
      <el-table v-loading="loading" :data="serviceFamilyList" @selection-change="handleSelectionChange" height="56vh">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200">
          <template slot-scope="scope">
            <el-button size="mini" type="text" @click="oepnFlowInfo(scope.row)">流转详情</el-button>
            <el-button type="text">
              <el-popconfirm title="确定审核通过吗?" @confirm="auditItem(scope.row, 2)">
                <el-button slot="reference" type="text" size="small">审核通过</el-button>
              </el-popconfirm>
            </el-button>
            <el-button type="text">
              <el-popover ref="popoverRef" placement="top" title="退回" width="200" trigger="click">
                <el-input
                  type="textarea"
                ...
                <div style="text-align: right; margin: 16px 0 0 0">
                  ...
                  <el-button type="primary" size="mini" @click="auditItem(scope.row, 3)">确定</el-button>
                </div>
                <el-button slot="reference" class="del" type="text" size="small">审核不通过</el-button>
              </el-popover>
            </el-button>
          </template>
        </el-table-column>
    ...
    />
    <FlowInfo v-if="flowInfoDialog" @close="flowInfoDialog = false" :propsData="flowInfoData"></FlowInfo>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listPetitionFeedbackLog({ ...this.queryParams, searchTime: null }).then((response) => {
    ...
    oepnFlowInfo(row) {
      this.flowInfoData = JSON.parse(JSON.stringify(row));
    ...
    // 审核
    auditItem(row, type) {
      if (type === 3 && this.backContent === '') {
        this.$message.error('请输入退回内容');
        return;
      }
      permFeedback({
        approvalResults: type,
        id: row.feedbackLogId,
        content: this.backContent,
      }).then((res) => {
        this.getList();
        type === 3 && this.closePopover();
      });
    },
    closePopover() {
