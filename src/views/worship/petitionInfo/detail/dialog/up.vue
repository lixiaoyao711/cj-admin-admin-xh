<template>
  ...
  >
    <section class="tree-box">
      ...
      <OftenPeople
        v-if="oftenPeopleDialog"
        @close="dialogVisible = false"
        type="1"
        @success="handleSelectionChange"
        :isRadio="true"
      />
    </section>
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button :loading="btnLoading" v-if="info.flowType === '3'" @click="handleSave">转 交</el-button>
...
import { petitionInfoPost } from '@/api/worship/petitionInfo';
...
export default {
  ...
  methods: {
    ...
    handleSave() {
      ...
      let flowContent = `请${this.checkData.nickName || this.checkData.label}处理,` + this.form.logContent;
      ...
      let obj = {
        id: this.info.id,
        petitionInfoFlowBo: {
          operType: this.info.flowType,
          flowContent,
          flowFileUrl: this.form.flowFileUrl,
          currentStepId: this.checkData.userId || this.checkData.id,
          currentStepName: this.checkData.nickName || this.checkData.label,
        },
      };
    ...
    handleSelectionChange(data, type) {
      ...
      if (type === 'org') {
        this.checkData.id = `D${this.checkData.id}`;
      }

      this.oftenPeopleDialog = false;
