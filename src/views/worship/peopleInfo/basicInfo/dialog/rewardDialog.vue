    ...
    getData() {
      if (this.propsId) {
        getPersonExpandById(this.propsId).then((res) => {
          ...
          this.rewards = this.form.expandContent.rewardsAndAwards
            ? this.form.expandContent.rewardsAndAwards.split(',')
            : [];
          this.punishment = this.form.expandContent.punishment ? this.form.expandContent.punishment.split(',') : [];
        });
      }
    },
    submitForm() {
