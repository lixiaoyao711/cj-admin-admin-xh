<template>
  <el-dialog :title="title" :visible.sync="open" width="700px" append-to-body @close="cancel">
    <el-form ref="form" :model="form.expandContent" :rules="rules" label-width="160px">
      <el-row>
        <el-form-item label="是否在校大学生入伍：" prop="">
          <el-radio-group v-model="form.expandContent.collegeStudents">
            <el-radio v-for="(item, index) in collegeStudents" :key="index" :label="item">{{ item }}</el-radio>
          </el-radio-group>
        </el-form-item>
      </el-row>
      <el-row>
        <el-form-item label="服役前在读学历：" prop="">
          <el-radio-group v-model="form.expandContent.educationInProgress">
            <el-radio v-for="(item, index) in educationInProgress" :key="index" :label="item">{{ item }}</el-radio>
          </el-radio-group>
        </el-form-item>
      </el-row>
      <el-row>
        <el-form-item label="已取得学历情况：" prop="">
          <el-radio-group v-model="form.expandContent.educationObtained">
            <el-radio v-for="(item, index) in educationInProgress" :key="index" :label="item">{{ item }}</el-radio>
          </el-radio-group>
        </el-form-item>
      </el-row>
      ...
      <el-row>
        <el-form-item label="学历取得时间：" prop="">
          <el-date-picker
            v-model="form.expandContent.obtainingTime"
            type="date"
            placeholder="选择日期"
      ...
      <el-row>
        <el-form-item label="已取得学位情况：" prop="">
          <el-radio-group v-model="form.expandContent.degreeObtained">
            <el-radio v-for="(item, index) in degreeObtained" :key="index" :label="item">{{ item }}</el-radio>
          </el-radio-group>
        </el-form-item>
      </el-row>
      <el-row>
        <el-form-item label="学位取得时间：" prop="">
          <el-date-picker
            v-model="form.expandContent.obtainedTime"
            type="date"
            placeholder="选择日期"
            value-format="yyyy-MM-dd"
          >
...
export default {
  ...
  data() {
    return {
      ...
      collegeStudents: ['是', '否'],
      educationInProgress: ['博士研究生', '硕士研究生', '大学本科', '大学专科', '中专', '高中', '初中及以下'],
      degreeObtained: ['博士', '硕士', '学士', '无'],
      customProps: {
        emitPath: true,
        label: 'name',
        value: 'adcode',
