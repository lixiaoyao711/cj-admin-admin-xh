  ...
  <div class="workInfoEditDialog">
    <el-dialog :title="title" :visible.sync="open" width="700px" append-to-body @close="cancel">
      <el-form ref="form" :model="form.expandContent" :rules="rules" label-width="140px">
        <el-row>
          <el-form-item label="就业状态：" prop="">
            <el-radio-group v-model="form.expandContent.employmentStatus">
              <el-radio v-for="(item, index) in employmentStatus" :key="index" :label="item">{{ item }}</el-radio>
            </el-radio-group>
          </el-form-item>
        </el-row>
        <el-row>
          <el-form-item label="个人年收入（元)：" prop="">
            <el-radio-group v-model="form.expandContent.personalAnnualIncome">
              <el-radio v-for="(item, index) in personalAnnualIncome" :key="index" :label="item">{{ item }}</el-radio>
            </el-radio-group>
          </el-form-item>
        </el-row>

        <el-row>
          <el-form-item label="家庭年收入（元）：" prop="">
            <el-input v-model="form.expandContent.annualHouseholdIncome" type="number"></el-input>
          </el-form-item>
        </el-row>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      personalAnnualIncome: ['0-10000', '10001-30000', '30001-50000', '50001-120000', '120001以上'],
      customProps: {
        emitPath: true,
        label: 'name',
        value: 'adcode',
