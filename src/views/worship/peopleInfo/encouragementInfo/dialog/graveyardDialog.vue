  ...
  <div class="workInfoEditDialog">
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body @close="cancel">
      <el-form ref="form" :model="form.expandContent" :rules="rules" label-width="170px">
        <el-row>
          <el-form-item label="是否符合条件：" prop="">
            <el-radio-group v-model="form.expandContent.whetherItMeetsConditions">
              <el-radio v-for="(item, index) in sex" :key="index" :label="item">{{ item }}</el-radio>
            </el-radio-group>
          </el-form-item>
        </el-row>
        <el-row>
          <el-form-item label="是否愿意申请军人公墓：" prop="">
            <el-radio-group v-model="form.expandContent.militaryCemetery">
              <el-radio v-for="(item, index) in sex" :key="index" :label="item">{{ item }}</el-radio>
            </el-radio-group>
          </el-form-item>
        </el-row>
