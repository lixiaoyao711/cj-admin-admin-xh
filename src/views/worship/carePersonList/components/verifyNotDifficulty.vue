<template>
  <div class="verifyNotDifficulty">
    ...
    >
      ...
      <!-- <el-form-item label="困难类型：" prop="verifyNotDifficulty">
        <el-select v-model="queryParams.verifyNotDifficulty" placeholder="请选择" clearable>
          <el-option
    ...
    <div class="table">
      <div class="table-title-box">
        <div class="title-name">核实非困难</div>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button
              ...
              @click="handleDel"
              :disabled="multiple"
              v-hasPermi="['worship:verifyNotDifficulty:transfer']"
              >从核实非困难移出</el-button
            >
          </el-col>
      ...
      >
        ...
        <el-table-column label="核实情况" align="center" prop="explanationDifficulties">
          <template slot-scope="scope">{{ scope.row.difficultInfo.explanationDifficulties }}</template>
        </el-table-column>
...
<script>
import { careInfoList, delCareInfo } from '@/api/worship/carePersonList';
  ...
  methods: {
    getList() {
      ...
      careInfoList(this.queryParams).then((response) => {
        this.verifyNotDifficultyList = response.rows;
