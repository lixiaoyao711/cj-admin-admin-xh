...
export default {
  ...
  methods: {
    setRank() {
      for (const key in this.propsData.colorMark) {
        this.rankList.push({
          name: key,
          count: this.propsData.colorMark[key],
        });
      }
    ...
    initPCharts() {
      ...
      // });
      let data = this.propsData.personCategory.map((item) => {
        return {
          name: item.name,
          value: item.count,
        };
