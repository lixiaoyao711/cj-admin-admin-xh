<template>
  <div class="three">
    <div class="item pie-box">
      <div class="pie-count">
        <router-link to="/petition/petitionInfo" class="count-item">
          <!-- <div class="count-item" @click="toPath('/petition/petitionInfo', '')"> -->
        ...
        <div class="list-item" v-for="(item, index) in rankList" :key="index">
          ...
          <div class="text">
            ...
            <div class="rank-box">
              <div
                class="ranks"
                :style="{ width: (item.count === 0 ? 0 : (item.count / rankSum) * 100).toFixed(2) + '%' }"
              ></div>
            </div>
            <div class="number">{{ item.count }}</div>
            <div class="percentage">{{ (item.count === 0 ? 0 : (item.count / rankSum) * 100).toFixed(2) }}%</div>
          </div>
        </div>
      </div>
    </div>
            ...
            itemStyle: {
              normal: {
                ...
                label: {
                  formatter: (val) => {
                    console.log(val);
                    return `${val.value}  (${val.value === 0 ? 0 : ((val.value / sum) * 100).toFixed(2)}%)`;
                  },
                  show: true,
